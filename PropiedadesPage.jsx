...
// Función para compartir, puedes meterla en tu PropiedadesPage.jsx
const compartirPropiedad = async (propiedad) => {
  const shareData = {
    title: `Propiedad en ${propiedad.direccion}`,
    text: `¡Mira esta propiedad que gestionamos! Zona: ${propiedad.idZona}`,
    url: window.location.href, // O la URL de la propiedad específica
  };

  try {
    if (navigator.share) {
      await navigator.share(shareData);
      toast.success('¡Contenido compartido!'); // Usando el toast de arriba ;)
    } else {
      // Por si están en un navegador muy antiguo que no lo soporta
...
// Ejemplo de botón en Tailwind para llamar a la función:
// <button onClick={() => compartirPropiedad(p)} className="text-blue-600 hover:text-blue-800 bg-blue-50 px-3 py-1 rounded-full text-xs font-semibold">Compartir</button>
